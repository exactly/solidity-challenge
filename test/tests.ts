...
describe('ETHPool', async () => {
  ...
  describe('User', async ()=> {
    ...
    it('should give correct rewards after some rewards cycles', async () => {
      ...
      // A withraws, B withdraws, A deposits, A withdraws 
      ...
      await ETHPool.functions.depositRewards({value: ethers.utils.parseEther("150.0")});
  
      await expect(await ETHPool.connect(alice).functions.withdraw()).to.changeEtherBalance(alice, ethers.utils.parseEther("300.0") );
      await expect(await ETHPool.connect(bob).functions.withdraw()).to.changeEtherBalance(bob, ethers.utils.parseEther("150.0") );
